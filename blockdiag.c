...
void blk_cm_print_info(blk_mat_complx *obj, char *title)
{
	int i;

	printf("\n\n%s : block diagonal matrix of dim %d in basis %d\n",title,obj->dim, obj->basis);
	printf("   %d BLOCKS, dimensions: ",obj->Nblocks);
	for (i=0; i<obj->Nblocks; i++) {
		printf(" %d,",obj->blk_dims[i]);
	}
	printf("\n");
}
...
//	blk_cm_print_info(U,"current prop");
//	blk_cm_print_info(dU,"increment prop");
			...
			if (sim->Hiso->basis == 0) {
				...
				if (wsp->Hiso != sim->Hiso) {
					if (wsp->Hiso->Nblocks != 1) {
						...
						wsp->Hiso->blk_dims[0] = sim->matdim;
						wsp->Hiso->Nblocks = 1;

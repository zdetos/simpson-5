...
void cv_elements_square(complx *vec) {
	int i;
	double re, im;
	for (i=1; i<=LEN(vec); i++) {
		re = vec[i].re;
		im = vec[i].im;
		vec[i].re = re*re-im*im;
		vec[i].im = 2*re*im;
	}
}
...
void cm_filter_proj(mat_complx *m, mat_complx *mask)
{
	complx z = cm_trace_adjoint(mask,m);
	mat_complx *dum = cm_dup(mask);
	cm_mulc(dum,z);
	cm_swap_innards_and_destroy(m,dum);
}

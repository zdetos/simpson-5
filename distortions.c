...
#include "OCroutines.h"
 ...
 int DOPs_slot() {
  ...
  /****
   * free slot in DOPs array
   ****/
void DOPs_free(int slot) {

    if (DOPs[slot] == NULL) {
    	fprintf(stderr,"DOPs error: cannot free DOP slot %d, it was not initiated\n",slot);
        exit(1);
    }
    free_complx_matrix(DOPs[slot]);
    DOPs[slot] = NULL;
}
 ...
 * implementation of Tcl create_distortion_operator routine
 ****/
int tclFreeDOP(ClientData data,Tcl_Interp* interp,int argc, Tcl_Obj *argv[])
{
	int slot;

	if ( argc != 2)
		return TclError(interp,"usage: free_distortion_operator <distortion operator> ");
	if (Tcl_GetIntFromObj(interp,argv[1],&slot) == TCL_ERROR)
		    return TclError(interp,"free_distortion_operator: argument 1 must be integer <distortion operator>");
	DOPs_free(slot);
...
int tclReconstructGradient(ClientData data,Tcl_Interp* interp,int argc, Tcl_Obj *argv[])
{
 ...
 void tclcmd_distortions(Tcl_Interp* interp) {
  ...
  Tcl_CreateObjCommand(interp,"free_distortion_operator",(Tcl_ObjCmdProc *)tclFreeDOP,(ClientData)NULL,(Tcl_CmdDeleteProc*)NULL);

...
void modnumbers(double at,double dt, int np, int *k, int *l)
{
   // at ... length of acq_block
   // dt ... dwell time
   // np ... number of points in fid to acquire
   double a,b;
   int m,n;
...
void modnumbers_mas(double a, double b, double c, int nbmax, int *k, int *l, int *m)
{
	...
	while ( (fabs(v[0]-v[1])>TINY || fabs(v[1]-v[2])>TINY) && (mul[ib]<=nbmax) ) {
		v[0] += incr[0];
		mul[0]++;
		if ( v[0]-v[1] > TINY) {
			v[1] += incr[1];
...
void direct_acqblock_freq(Tcl_Interp *interp,Tcl_Obj *obj,Sim_info *sim,Sim_wsp *wsp)
{
	...
	k = m = 1;
	if (sim->taur > 0) {
		modnumbers(period,sim->taur,1000000,&k,&m);
		if (verbose & VERBOSE_ACQBLOCK) {
			printf("acq_block synchronization info: %d * acq_block(%g us) = %d * rotor period(%g us)\n",k,period,m,sim->taur);
		}
		period *= k;
		if (fabs(period-m*sim->taur)>TINY) {
			fprintf(stderr,"Error: acq_block: synchronization with rotation not found\n");
			exit(1);
		}
	}
	freqT = 2*M_PI*1e6/period;
...
double direct_props(Tcl_Interp *interp,Tcl_Obj *obj,Sim_info *sim,Sim_wsp *wsp)
{
	...
	k = m = 1;
	if (sim->taur > 0) {
		modnumbers(period,sim->taur,1000000,&k,&m); // assuming 1000000 is number big enough
		if (verbose & VERBOSE_ACQBLOCK) {
			printf("acq_block synchronization info: %d * acq_block(%g us) = %d * rotor period(%g us)\n",k,period,m,sim->taur);
		}
		period *= k;
		if (fabs(period-m*sim->taur)>TINY) {
			fprintf(stderr,"Error: acq_block - can not find synchronization with MAS\n");
			exit(1);
		}
	}
	wsp->t = t0; // reset time counter

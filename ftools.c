...
int tclFSet(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  if (argc < 4 || (argc % 2) != 0) 
    return TclError(interp,"Usage: fset <data set> ?-ref v?  ?-sw v? ?-type (spe|fid)? ?-ni v? ?-ref1 v?  ?-sw1 v? ?-sfrq v? ?-sfrq1 v?");
  ...
  for (ac=2;ac<argc;ac += 2) {
    ...
    } else if (!strcmp(argv[ac],"-sfrq")) {
      if (Tcl_GetDouble(interp,argv[ac+1],&(f->sfrq)) == TCL_ERROR)
        return TclError(interp,"fset: argument %d must be double",ac+1);

    } else if (!strcmp(argv[ac],"-sfrq1")) {
      if (Tcl_GetDouble(interp,argv[ac+1],&(f->sfrq1)) == TCL_ERROR)
        return TclError(interp,"fset: argument %d must be double",ac+1);
...
int tclFGet(ClientData data,Tcl_Interp* interp,int argc, char *argv[])
{
  ...
  if (argc != 3)
    return TclError(interp,"Usage: <result> fget <data set> [-ref | -sw | -np | -ni | -sw1 | -ref1 | -type | -nelem | -sfrq | -sfrq1]");
  ...
  } else if (!strcmp(argv[2],"-sfrq")) {
    TclSetResult(interp,"%e",fd[fidN]->sfrq);
  } else if (!strcmp(argv[2],"-sfrq1")) {
    TclSetResult(interp,"%e",fd[fidN]->sfrq1);

  ...
  while (!feof(fp) && !got_data) {
    ...
    if (q) {
       *q=0; q++;
       ...
       } else if (!strcmp(com,"SFRQ")) {
          if (sscanf(val,"%lg",&fd->sfrq) != 1) {
            fprintf(stderr,"FD_read: unable to convert '%s' to double in file '%s'\n",val,fname);
            exit(1);
          }
       } else if (!strcmp(com,"SFRQ1")) {
          if (sscanf(val,"%lg",&fd->sfrq1) != 1) {
            fprintf(stderr,"FD_read: unable to convert '%s' to double in file '%s'\n",val,fname);
            exit(1);
          }
...
void FD_write_raw_bin(char* fname,FD* fd)
{
  ...
  if (fd->type == FD_TYPE_SPE) {
    if (ni > 1) {
     /* for (j=ni+1; j>1; j--) {
        if (j > ni) jj = 1;	else jj = j;
        no = (jj-1)*fd->np;
        for (i=no+fd->np+1;i>no+1;i--) {
	      if (i > no+fd->np) ii = 1; else ii = i;
          f = fd->data[ii].re;
          fwrite(&f, sizeof(float), 1, fp);
        }
        for (i=no+fd->np;i>no;i--) {
        ...
        for (j=1; j<= ni; j++) {
          no = (j-1)*fd->np+1;
          for (i=no;i<no+fd->np;i++) {
            f = (float)fd->data[i].re;
            fwrite(&f, sizeof(float), 1, fp);
          }
          for (i=no;i<no+fd->np;i++) {
            f = (float)fd->data[i].im;
            fwrite(&f, sizeof(float), 1, fp);
          }
        }

...
void thread_work(int thread_id, Tcl_Interp *interp){
  ...
  while (1) {
	  ...
	  irf = nrf_start+1;
	  iz = nz_start+1;
	  // hack rfdata with rfmap
	  if (sim->rfmap != NULL) { //rfmap replaces rfdata (sure it was rfprof_file='none')
		  weight = sim->crdata[icr].weight * sim->rfmap->weight[irf-1] * thrd.ave_weight[ave_start];
		  wsp->rffac = sim->rfdata[1]; // this is always defined and here contains all ones
		  wsp->rf_idx = irf-1; // rfmap->loop index is zero-based
	  } else { //ordinary rfdata from rfprof_file
...
//	  wsp->rffac = sim->rfdata[irf];
	  ...
	  sim_calcfid(sim, wsp);
	  //printf("worker (%i/%i): irf=%d, fid[1].re = %f\n", thread_id+1,glob_info.mpi_rank+1, irf, wsp->fid[1].re);
	  if (sim->oc_gamma_free && OCpar.isinit) { // special processing: after averaging over gamma inside sim_calcfid take the square of target function
		  //printf("thread worker (%i/%i): inside oc_gamma_free\n", thread_id+1,glob_info.mpi_rank+1);
		  if (OCpar.gradmode) { // we are in OC gradient mode
			  int qq;
			  complx zzz;
			  //cv_print(wsp->OC_phivals,"Original OC_phivals");
			  //cv_print(wsp->fid,"Original fid");
			  for (qq=0; qq<LEN(wsp->OC_phivals); qq++) {
				  zzz.re = wsp->OC_phivals[qq+1].re*2.0;
				  zzz.im = wsp->OC_phivals[qq+1].im*2.0;
				  cblas_zscal(sim->ntot,&zzz,(wsp->fid)+1+qq*sim->ntot,1);
			  }
			  cv_elements_square(wsp->OC_phivals);
			  //cv_print(wsp->OC_phivals,"New OC_phivals");
			  //cv_print(wsp->fid,"New fid");
		  } else { // we are in target function evaluation
			  //cv_print(wsp->fid,"Original fid");
			  cv_elements_square(wsp->fid);
			  //cv_print(wsp->fid,"New fid");
		  }
	  }
	  cv_multod(fidsum, wsp->fid, weight);
	  if (collect_phivals) cv_multod(phivals,wsp->OC_phivals,weight);
	  //printf("worker (%i/%i): irf=%d, fidsum[1].re = %f\n", thread_id+1,glob_info.mpi_rank+1, irf, fidsum[1].re);
...
void simpson_thread_slave(void *thr_id)
{
	...
	//printf("simpson_thread_slave %d after Tcl Init\n",thread_id);
	TclSlaveInterpInit(interp);
	//printf("simpson_thread_slave %d after TclSlaveInterp Init\n",thread_id);
...
void simpson_common_work(Tcl_Interp *interp, char *state, int *ints_out, double *dbls_out, complx **fidsumptr, complx **phivalsptr)
{
	...
	case INTERPOL_NOT_USED: // no interpolation
		...
		thrd.nrf = nrf;
		thrd.nz = nz;
		// hack rfprof with rfmap
		if (sim->rfmap != NULL) { // sure it was rfprof_file = 'none'
			thrd.Ntot *= sim->rfmap->loop_size;
			thrd.nrf *= sim->rfmap->loop_size; // it was 1
		}
		thrd.Nacq = 0;
		thrd.phase = 0;
		thrd.zvals = zvals;
